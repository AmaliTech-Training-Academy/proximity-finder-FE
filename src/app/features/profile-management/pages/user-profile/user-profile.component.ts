...
export class UserProfileComponent implements OnInit {
  ...
  onSubmit() {
    if (this.userForm.valid) {
      ...
      const updatedClient: ProviderData = {
        ...this.client,
        userName: name ?? '',
        mobileNumber: phone ?? '',
      };
      ...
      if (
        updatedClient.userName !== this.client.userName ||
        updatedClient.mobileNumber !== this.client.mobileNumber
      ) {
  ...
  openDialog() {
    const dialogComponent = this.isDeleteModal
    ...
    const dialogRef = this.dialog.open(dialogComponent, {
      data: this.isDeleteModal
        ? {
            title: 'Delete item',
            message:
              'Are you sure you want to delete this account? Deleting account will remove it permanently from your linked accounts',
            type: 'delete',
            confirmText: 'Delete',
            cancelText: 'Cancel',
          }
  ...
  updateAccount() {
    if (this.accountInfoForm.valid) {
      ...
      if (this.selectedAccount) {
        this.profileService
          .editPaymentAccount(updatedAccount, this.selectedAccount.id)
          .subscribe({
            next: () => {
  ...
  deleteAccount() {
    if (this.selectedAccount) {
      this.profileService
        .deletePaymentAccount(this.selectedAccount.id)
        .subscribe({
          next: (account) => {
